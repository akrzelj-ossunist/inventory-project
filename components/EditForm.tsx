...
const EditForm: React.FC<{ setSideNavView: (value: boolean) => void }> = ({
  setSideNavView,
}) => {
  ...
  return (
    <div className="m-10">
      <div className="flex justify-between">
        ...
        <button onClick={() => setSideNavView(false)} className="text-blue-500">
          Close
        </button>
      </div>
      <Formik
            ...
            <Form>
              ...
              <div className="flex flex-col m-5 ml-0">
                ...
                {errors.serialNumber && touched.serialNumber && (
                  <label className="text-red-500">{errors.serialNumber}</label>
                )}
              </div>
              <div className="flex flex-col m-5 mb-10 ml-0">
                ...
                {errors.description && touched.description && (
                  <label className="text-red-500">{errors.description}</label>
                )}
              </div>
              <div>
                <div className="flex">
                  <p className="font-bold text-2xl">Warranties and images</p>
                  <p className="text-blue-500 underline">(optional)</p>
                </div>
                <ImageDropzone />
              </div>
              <div className="flex m-5 ml-0 items-center">
                <Field type="checkbox" className="w-6 h-6 rounded-xl mr-2" />
